...
object BrewProgressDecoder {
    ...
    fun decode(dataHex: String, catalog: MachineCatalog): BrewProgress {
        ...
        val state = b[0]
        return if (state in catalog.dispensingStates) {
            BrewProgress.Dispensing(doneMl = b[4] * 5, totalMl = b[5] * 5, percent = minOf(b[14], 100))
        } else {
            BrewProgress.Phase(state, catalog.brewStateNames[state] ?: "State 0x%02X".format(state))
        }
    }
}
