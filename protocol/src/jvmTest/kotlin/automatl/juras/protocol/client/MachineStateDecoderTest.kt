...
class MachineStateDecoderTest {
    ...
    fun `ready state decodes coffee ready without fill water`() {
        val state = MachineStateDecoder.decode("000400000C0000", MachineCatalog.forModel("EF1030"))
    ...
    fun `tank-out state decodes fill water without coffee ready`() {
        val state = MachineStateDecoder.decode("400000000C0000", MachineCatalog.forModel("EF1030"))
    ...
    fun `payload too short does not throw and decodes what it can`() {
        val state = MachineStateDecoder.decode("40", MachineCatalog.forModel("EF1030"))
